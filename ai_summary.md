...
> 経緯は `changelog.md`、設計と恒久的な制約は `docs/v1-design.md`（v1.12。絶対制約は §0・付録B）を参照。

- **最終更新日**: 2026-09-18
- **プロジェクトの目的**: Mac mini（M4・24GB・MPS）上で動く完全ローカルの音声付き動画生成アプリ。初心者がブラウザ（Gradio・日本語UI・localhost限定）から MiniMax-H3 動画を生成できるようにする。
- **現在フェーズ**: **V1 全フェーズ（P0〜P8）完了。OSS として公開済み**
  （<https://github.com/atelier-lab-jp/atelier-h3-local-studio>。v1.0.0 を 2026-08-21 に公開。
  詳細は「V1 完成後の状態」）。**2026-09-18 に P6.1（連結動画の1080p高品質化が失敗する
  不具合）を修正し、v1.0.1 として公開**（下記）

## P6.1 修正: 連結動画の高品質化の検証基準を実測値へ（2026-09-18・v1.0.1 として公開）

連結動画（例: `cm_20260918_223331_qhrd`・124f×6本）の1080p高品質化が
「フレーム数が想定と違います（実測 745 / 想定 744）」で必ず失敗していた。
**連結側は §10.6.2 のとおり正常**（AAC 量子化による音声超過を `-r 24` が重複フレームで
埋め、名目合計より最大 `ceil((n−1)×0.768)` 枚多いのが正しい）で、台帳は名目値 744 を
記録する一方、高品質化側が名目値との厳密一致を要求していた契約不一致が原因。

- **修正**（`app/core/upscale_service.py` のみ・設計書 §26.5 v1.12）: 開始時に入力を
  `decode_probe()` で1回だけ実測し、**ワーカーの `--expected-frames`・進捗の分母・
  出力検証で実測値を一貫して使う**。入力と出力の厳密一致検査は維持（±N許容・末尾
  トリム・想定値の付け替えはしない）。計測不能・24fps 不整合は名目値へフォールバック
  せずワーカー起動前に日本語エラー
- チェーン連結（`c_*`）の高品質化が終端クリップ単体の枚数（例: 124）を想定値として
  渡され2本以上で必ず失敗する潜在不具合も、同じ変更で解消
- `tests/fixtures/fake_upscale_worker.py` を「入力を実際に数える」忠実な代役へ変更し、
  この不具合を検出できる試験6件を追加（修正前は実機と同一文言で失敗することを確認済み）
- **実機確認済み**（M4/MPS・実モデル・通常の AppService 経路・隔離 data_root）:
  `cm_20260918_223331_qhrd` が 91.7秒で成功。1920×1080・24fps・**745フレーム**・
  映像 31.041667秒、音声ストリーム MD5 一致（stream copy）、A/V 開始位置・長さの差 0、
  元動画 MD5 不変、中間ファイル・孤児プロセスなし
- **ユーザーが実機UIからも成功を確認済み（2026-09-18・修正反映後）**: アプリ再起動後、
  ③「完成・編集」から `cm_20260918_223331_qhrd` の高品質化を実行し、**完了表示と
  正式な出力ファイル名の表示**を確認した（音声の聴取・全編再生の確認は含まない）
- 残件（意図的に未変更）: ③チェーン連結行の枚数・長さ表示（終端クリップ単体の値）／
  1080p成果物一覧の枚数表示（名目値）／ワーカーの早期中断／台帳 `num_frames_total` の
  意味（名目合計のまま）
...
生成エンジン・キュー契約・履歴スキーマ・設計契約は**無変更**（設計書も更新不要）。
...
設計書 `docs/v1-design.md` は **v1.12**。P0（基盤）→ P0.5（バックエンド境界）→ P1（履歴・キュー・Engine契約・MockEngine・新規生成タブ）→ P2（実機エンジン）→ P3（キューと進捗）→ P4（継続生成・連結）→ P5（LANモード・総仕上げ）→ P5.1〜P5.3（UI改善・任意順序連結・整理）→ P6（1080p高品質化）→ P7（履歴閲覧専用化）→ **P8（開始画像）** まで**全フェーズ完了**。
...
- **1486 passed + 1 skipped + 1 xpassed**（P4 時点は 625+1。P5 で 367 件、P5.1 で 15 件、
...
- **S9（Wi-Fi 断での完走）は未実施＝ユーザーによる手動確認項目**（OSS公開のBLOCKERとしては扱わない）。P2 では代替として (1) worker.log の `[√] Skip download and load only pre-downloaded model files` (2) モデルファイルの mtime/サイズ変化ゼロ、で追加ダウンロードが無いことを確認した
- 再起動中にアプリを終了すると体感の終了時間が10〜15秒になりうる（無応答ワーカーの terminate 待ちが積み上がるため。孤児ワーカーは残らない）

## V1 完成後の状態（2026-09-18 時点・OSS 公開済み）

**V1.0.0 のコードは完成し、実機での最終確認も完了し、OSS として公開済み。**

- **公開先**: <https://github.com/atelier-lab-jp/atelier-h3-local-studio>（Public・
  Apache-2.0・`THIRD-PARTY-NOTICES.md` 同梱・author/tagger は GitHub noreply アドレス）
- **v1.0.0 を 2026-08-21 に公開**（タグ `v1.0.0` = `948642f`。GitHub Release は作成せず
  タグのみで公開した）
- **v1.0.1 を 2026-09-18 に公開**（P6.1 修正。修正コミット `e77a558` ＋公開準備コミット。
  この版から **GitHub Release も併せて公開**する方式にした）
- **実 iPhone での LANモード最終確認は 2026-08-20 に完了**。iPhone からの接続・
  新規生成タブからのプロンプト投入・実モデルでの生成完走・完成動画の iPhone 上での再生・
  「完成・編集」タブと「履歴」タブへの反映・レスポンシブ表示を確認した
  （ジョブ `v_20260820_235040_brtj`: 56フレーム・24fps・4ステップ・約2.33秒・
  seed 262872212・処理時間 約6分38秒・SUCCESS）
- 公開時に適用済み: LICENSE（Apache-2.0 公式本文）・
  `THIRD-PARTY-NOTICES.md`（DiffSynth-Studio / Real-ESRGAN の帰属とモデルライセンス情報）・
  README の公開向け整備（uv 必須・モックでの動作確認導線・実モデル準備章・ライセンス節・
  更新手順）・由来コードへの帰属コメント・author email の GitHub noreply 化
...
1. 設計書の絶対制約 — §0（前提）と付録B「変更しないもの」一覧（DiffSynth 読み取り専用・V1固定仕様）、§18 のフェーズ境界
2. 本書の「V1 完成後の状態」と「残っている警告・未解決事項」
3. `.venv/bin/python -m pytest tests/ -q` が **1486 passed + 1 skipped + 1 xpassed** であること
