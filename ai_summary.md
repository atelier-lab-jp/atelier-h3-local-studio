...
> 経緯は `changelog.md`、設計と恒久的な制約は `docs/v1-design.md`（v1.11。絶対制約は §0・付録B）を参照。

- **最終更新日**: 2026-08-21
- **プロジェクトの目的**: Mac mini（M4・24GB・MPS）上で動く完全ローカルの音声付き動画生成アプリ。初心者がブラウザ（Gradio・日本語UI・localhost限定）から MiniMax-H3 動画を生成できるようにする。
- **現在フェーズ**: **V1 全フェーズ（P0〜P8）完了。実 iPhone での LANモード最終確認も完了（2026-08-20）。現在は OSS 公開準備中**（詳細は「V1 完成後の状態」）
...
設計書 `docs/v1-design.md` は **v1.11**。P0（基盤）→ P0.5（バックエンド境界）→ P1（履歴・キュー・Engine契約・MockEngine・新規生成タブ）→ P2（実機エンジン）→ P3（キューと進捗）→ P4（継続生成・連結）→ P5（LANモード・総仕上げ）→ P5.1〜P5.3（UI改善・任意順序連結・整理）→ P6（1080p高品質化）→ P7（履歴閲覧専用化）→ **P8（開始画像）** まで**全フェーズ完了**。
