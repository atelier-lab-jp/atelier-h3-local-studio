...
**決定D20: 厳密な資産管理はしない。** このアプリは動画資産管理システムではなく、
「気軽に作って確かめ、気に入った成果物だけ残す」道具。設計は `docs/v1-design.md` §25。

- **不採用**（作らない）: `trash_manifest.json`／`jsonstore.py`／tombstone／復元情報の永続化／
  復元UI／依存関係検査／参照先一覧／カスケード削除／非表示フィルタ／削除済みIDの永続管理／
  内部JSONからの物理削除
- **表示の正本はファイルの実在** — 動画ファイルが無い記録は③④の一覧・候補から外す。
  内部の `history.json`・`concat_manifest.json` は**消さない**。除外リストを持たないので
  **正式パスへ戻せば次の更新で自然に再表示**される。失敗・取消・中断は従来どおり残す
- **アプリ内ゴミ箱** — 実在動画を選んだときだけ現れる確認チェック＋赤ボタンで
  `data_root/trash` へ**移動するだけ**（配信対象外・実行時のみ作成・復元UIなし）
- **依存関係は管理しない** — 親を消しても子は残り、素材を消しても既存の連結動画は
  残って再生できる。素材が要る操作は既存の検証境界が断る
- 新規モジュールは `app/core/trash_service.py`（ファイル移動と境界検証だけ・約160行）
