...
- **現在フェーズ**: **V1 全フェーズ（P0〜P8）完了。実 iPhone での LANモード最終確認も完了（2026-08-20）。現在は OSS 公開準備中**（詳細は「V1 完成後の状態」）。**2026-09-18 に P6.1（連結動画の1080p高品質化が失敗する不具合）を修正**（下記）

## P6.1 修正: 連結動画の高品質化の検証基準を実測値へ（2026-09-18）
...
## V1 完成後の状態（2026-08-21 時点）

**V1.0.0 のコードは完成し、実機での最終確認も完了した。**
...
- **Git 管理は開始済み**（`main` ブランチにコミットあり。経緯は `changelog.md`）
- **現在は OSS 公開準備中**。適用済み: LICENSE（Apache-2.0 公式本文）・
  `THIRD-PARTY-NOTICES.md`（DiffSynth-Studio / Real-ESRGAN の帰属とモデルライセンス情報）・
  README の公開向け整備（uv 必須・モックでの動作確認導線・実モデル準備章・ライセンス節）・
  由来コードへの帰属コメント。
  残る公開前作業: 過去コミットの author email を GitHub noreply へ書き換え →
  `v1.0.0` タグの付け直し → GitHub リポジトリの作り直し → push → Public 化
  （いずれもユーザー承認のもと段階的に実施する）
