...
開発中はフェーズ表記（`[Unreleased]` ＋日付）で管理してきた。
**2026-08-21 の `v1.0.0`（タグ `948642f`）までの `[Unreleased]` 見出しは、
すべて v1.0.0 に含まれる**（見出しは開発当時の記録としてそのまま残す）。
v1.0.1 以降は版名の見出しで記録する。

---

## [v1.0.1] 2026-09-18 — P6.1: 連結動画の1080p高品質化が失敗する不具合の修正

`cm_20260918_223331_qhrd`（124フレーム×6本の指定順連結）の1080p高品質化が、745枚
すべてを処理し終えた後に「フレーム数が想定と違います（実測 745 / 想定 744）」で
必ず失敗していた（2026-09-02 に同一素材で3回、2026-09-18 に1回、全て同一文言）。

### 原因（調査 2026-09-18・契約不一致）

- **連結側は正しい** — 各クリップの音声は AAC の 1024 サンプル量子化で映像より最大
  32ms 長く、concat フィルタは各セグメントを `max(映像長, 音声長)` だけ進めるため、
  `-r 24` が境界の隙間を重複フレームで埋める。§10.6.2 はこれを正当と定め、連結の
  検証は名目合計＋境界数比例の超過を**合格**させる（今回の6本は境界5×0.200f＝
  ちょうど+1枚。連結MP4は実測745枚/31.041667秒で、9/2 と 9/18 の成果物は MD5 一致）
- **台帳は名目値を記録** — `concat_manifest.json` の `num_frames_total` は素材の
  名目合計（744）。UI の「31.00秒・744フレーム」もこの値
- **高品質化側だけが名目値との厳密一致を要求** — 台帳値を `--expected-frames` に渡し、
  ワーカー（`upscale_worker.py`）が実際に読めた枚数（745）と突き合わせて失敗していた
- 併発: チェーン連結（`c_*`）の一覧行は**終端クリップ単体の枚数**しか持たないため、
  チェーン連結の高品質化は2本以上で必ず失敗する状態だった（ログ上は未発生・潜在）

### Fixed
- **検証基準を「入力動画の実測フレーム数」へ変更**（`app/core/upscale_service.py`・
  設計書 §26.5 v1.12）。開始時に `decode_probe()`（fps 変換なしのフルデコード）で
  1回だけ計測し、①ワーカーの `--expected-frames` ②進捗の分母 ③昇格前の出力検証、
  で同じ値を一貫して使う。計測結果はジョブ内で引き回し（入力の重複デコードを
  3回→1回に削減）、ジョブをまたぐキャッシュは持たない
- **入力と出力の厳密一致検査は維持**（±1の許容・末尾トリム・想定値の付け替えは
  していない）。ワーカーの自己検査も従来どおり働く（実測値を渡すようになっただけ）
- 実測できない入力（映像が読めない・枚数/秒数が取れない・実測枚数÷24fps が実測
  秒数と±0.5秒超で食い違う）は、名目値へ**黙ってフォールバックせず**ワーカー起動前に
  日本語エラーで中止する
- チェーン連結の高品質化も同じ変更で解消（名目値は表示にしか使わなくなった）

### Changed
- `tests/fixtures/fake_upscale_worker.py` — 本物と同じく**入力を実際に数えて**書き出し、
  枚数が `--expected-frames` と食い違えば本物と同一文言・同一終了コードで失敗する
  忠実な代役へ変更（従来は渡された値を鵜呑みにするだけで、この不具合を構造的に
  検出できなかった）。`FAKE_UPSCALE_FRAMES` は「自己検査をすり抜けた想定」となり、
  サービス側の出力検証（第2層）のテスト用として意味を保つ

### Added（試験 6件・`tests/test_upscale_service.py`）
- 指定順連結相当: `mock_124.mp4`×5 を本番の `concat_reencode()` で連結（名目620・
  実測621）→ 高品質化が成功し、進捗分母・出力枚数とも実測値になる。
  **修正前はこのテストが実機と同一文言（実測 621 / 想定 620）で失敗することを確認済み**
- チェーン連結相当: `mock_56.mp4`×2（実測112）へ名目56（終端クリップ単体）を渡しても成功
- ワーカーへ渡る `--expected-frames` が名目値ではなく実測値であることを引数で確認
- ワーカー出力が実測より**少ない**場合の拒否（多い場合は既存試験）
- 計測不能な入力ではワーカーを起動せず失敗し、次の1件を普通に受け付ける
- 24fps と整合しない入力（30fps・90枚）を開始前に拒否する

### Verified（実機・M4/MPS・実モデル realesr-animevideov3）
- 全試験 **1486 passed + 1 skipped + 1 xpassed**（修正前 1480+1+1。+6 は今回の追加分。
  既存の失敗なし・skip/xpass は従来と同一）
- `cm_20260918_223331_qhrd` を通常の AppService 経路（`start_upscale` → 実ワーカー →
  stream copy mux → 検証 → 原子的昇格）で処理し **91.7秒で成功**。
  隔離 data_root（`data/tmp/p6fix_verify/`。稼働中アプリとロック・ファイルを共有しない）
  への実データコピー（MD5 一致）で実施
- 出力: H.264 High/yuv420p・**1920×1080・24fps・745フレーム**・映像 31.041667秒
- 進捗の分母は開始1秒未満の仮表示（744）の後、実測の **745** で完走
- 音声: `-map 0:a:0 -c copy -f adts` で取り出したストリームの **MD5 が入出力で完全一致**
  （再エンコードなし・末尾維持）。映像/音声の開始位置・ストリーム長も
  入出力で差 0.000000秒（映像 31.041667s・音声 31.104000s・音声開始 -0.032s を維持）
- 元の連結MP4は処理前後で **MD5 不変**（コピー・原本とも `2f533e9f…`）
- 成果物は `upscaled_rows()`・`find_row(kind="upscaled")` の通常経路で認識され、
  2回目の開始は「1080p高品質版はすでにあります」で拒否（正式昇格の認識）
- 中間ファイル（`.partial`・`.video.mp4`）残留なし・孤児プロセスなし
- 冒頭（frame 0）・最終連結境界付近（620/621）・末尾（744）の抽出フレームを目視し、
  1920×1080 の破綻ない高品質化を確認（動画としての再生・聴取は未実施＝ユーザー確認項目）
- **ユーザー確認（2026-09-18・修正反映後）**: アプリ再起動後、実機UI（③完成・編集）から
  `cm_20260918_223331_qhrd` の高品質化を実行し、**完了と正式な出力ファイル名の表示**を
  確認済み（音声の聴取・全編再生の確認は含まない）

### 残件（今回のスコープ外・意図的に変更していない）
- ③のチェーン連結行の表示（`num_frames`・長さが終端クリップ単体の値になる）
- 1080p成果物の一覧行のフレーム数表示（元動画の名目値のまま。実測745に対し744と出る）
- ワーカーの `processed > expected` 早期中断（真の異常時に全枚数を処理してから
  失敗する挙動の短縮。実測基準では実害がないため見送り）
- 台帳 `num_frames_total` の意味は名目合計のまま（既存エントリとの互換を維持）

### 公開（v1.0.1）
- アプリ版数を **1.0.1** へ（`pyproject.toml`・`config/config.example.toml`・
  `uv.lock` は自プロジェクトの版数行のみ更新で**依存は不変**）。
  設計書の文書バージョン（v1.12）はアプリ版数とは別の系列
- README §2 に**更新手順**を追記（`git pull` 利用者と ZIP 利用者を区別。
  `data/`・`config/config.toml`・取得済みモデルを失わない手順）
- タグ `v1.0.1` ＋ GitHub Release として公開（このリポジトリ初の GitHub Release。
  v1.0.0 はタグのみで公開していた）
